...
let CSAEarliestArrivalDynamic = class CSAEarliestArrivalDynamic {
    ...
    async plan(query) {
        ...
        const startLocation = this.placeLocation(query.from[0], trees);
        this.eventBus.emit(EventType_1.default.Partition, startLocation.id);
        ...
        const mergedQueue = new MergeIterator_1.default([connectionsIterator, footpathsQueue], ConnectionSelectors_1.forwardsConnectionSelector);
        const queryState = {
            finalReachableStops: {},
            profilesByStop: {},
    ...
    async updateProfile(state, query, connection) {
        ...
        */
        ...
        if (state.profilesByStop[connection.arrivalStop].arrivalTime === Infinity) {
            this.eventBus.emit(EventType_1.default.ReachableID, connection.arrivalStop);
            // this is the first time we reach this stop
            const stop = await this.stopsProvider.getStopById(connection.arrivalStop);
            const cluster = this.placeLocation(stop, state.trees);
            if (cluster && !this.reachedClusters.has(cluster.id)) {
                ...
                this.eventBus.emit(EventType_1.default.Partition, cluster.id);
                ...
                // this one got popped from the queue, place it back
                const aaa = state.mergedQueue.values[state.mergedQueue.values.length - 1];
                if (aaa) {
                    state.footpathsQueue.write(aaa);
                }
                state.footpathsQueue.removeAllListeners();
                state.mergedQueue.removeAllListeners();
                state.connectionsIterator = newConnectionIterator;
                state.mergedQueue = new MergeIterator_1.default([newConnectionIterator, state.footpathsQueue], ConnectionSelectors_1.forwardsConnectionSelector);
            }
        }
            ...
            for (const reachableStop of reachableStops) {
                ...
                if (duration && stop.id) {
                    ...
                    if (transferTentativeArrival > newArrivalTime.getTime()
                        ...
                        if (tripId === "TRANSFER_TO:https://data.delijn.be/stops/308648@1604392620000") { }
