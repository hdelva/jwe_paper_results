...
interface IQueryState {
    ...
    mergedQueue: MergeIterator<IConnection>;
    trees: HypermediaTree[];
    lowerBoundDate: Date;
}
...
export default class CSAEarliestArrivalDynamic implements IPublicTransportPlanner {
    ...
    public async plan(query: IResolvedQuery): Promise<AsyncIterator<IPath>> {
        ...
        this.eventBus.emit(EventType.Partition, startLocation.id);
    ...
    protected async updateProfile(state: IQueryState, query: IResolvedQuery, connection: IConnection) {
        ...
        */
        ...
        if (state.profilesByStop[connection.arrivalStop].arrivalTime === Infinity) {
            this.eventBus.emit(EventType.ReachableID, connection.arrivalStop);
            ...
            if (cluster && !this.reachedClusters.has(cluster.id)) {
                ...
                this.eventBus.emit(EventType.Partition, cluster.id);
